...
import { GameState, IMatchStatus, MatchStatus } from '../../../../shared/clients';
...
export class CurrentGameListPageComponent implements OnInit, OnDestroy {
  matches: RollList<IMatchStatus>;

  constructor(private hubService: HubService, private matchService: MatchService) { }

  private onMatchCreated(matchStatus: IMatchStatus) {
    this.matches.elements.splice(0, 0, matchStatus);
  }

  private onMatchUpdated(matchStatus: IMatchStatus) {
  ...
  ngOnInit() {
    this.matchService.getCurrentMatches(null, 25).subscribe(res => {
      this.matches = new RollList<IMatchStatus>(res.data, res.hasMoreLeft);
